...
def card(x, show_editorial=False):
    ...
    if show_editorial:
        ...
        if st.checkbox("📢 Include in this week's Teams bulletin", value=chosen, key=f"sel_{x.get('id')}"):
...
with tabs[0]:
    ...
    for label, data in [
        ...
        if data:
            for x in ranked(data)[:5]: card(x, show_editorial=True)
        else:
...
with tabs[1]:
    ...
    if new_items:
        for x in ranked(new_items): card(x, show_editorial=True)
    else:
...
with tabs[2]:
    ...
    for x in top: card(x, show_editorial=True)
...
with tabs[3]:
    ...
    for x in ranked(data): card(x, show_editorial=True)
...
with tabs[4]:
    ...
    for x in data: card(x, show_editorial=True)
